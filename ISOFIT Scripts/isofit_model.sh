...
fitsdump=$(python2 -c "import fits_center; fits_center.getFitsInfo(\"$filename\")")
fitsarr=($fitsdump)
x0=${fitsarr[0]}
y0=${fitsarr[1]}
maxsma=${fitsarr[2]}

posangle=$(python2 -c "import posAngle; posAngle.getPosAng(\"$1\", \"gal_list.cat\")")

./isofit_cl_p1.sh $x0 $y0 $posangle $maxsma $filename $modtab

bgdump=$(python2 -c "import background; background.getBgrd(\"$modtab\")")
bgarr=($bgdump)
bg=${bgarr[${#bgarr[@]}-1]}
